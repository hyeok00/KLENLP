...
<body>
   ...
   <form class="bottom" action="searchAction.jsp?">
		...
		<button id="search_btn" name="category" value="3">검색</button>
	</form>
   ...
   <div class="pagination">
      ...
      %>
      <a href="table3.jsp?curPage=<%=curPage%>">◀</a>
      <%
         for (int i = stratPage; i < stratPage + pageNum; i++) {
      %>
      <a href="table3.jsp?curPage=<%=i%>"><%=i%></a>
      <%
         if (i >= totalPage)
               break;
         }
...
<script>
   function click_write(){
      ...
      if(<%=session.getAttribute("id") != null%>){
         location.href='/Web_Project/include/iframe/write.jsp?category=3'
      }else  {
         alert('로그인 후 이용가능합니다.');
      }   
    }
