...
<body>
		...
		<tbody class="table_tbody">
			<%
				...
				for (int i = 0; i <dtos.size(); i++) {
					...
					out.print("<tr>");
					out.print("<td>" + no + "</td>");
					out.print("<td> <a href=/Web_Project/include/iframe/borad.jsp?no=" + no + "&category=" + type + ">"
							+ title + "</a></td>");
	...
	<form class="bottom" action="searchAction.jsp?">
		...
		<button id="search_btn" name="category" value="2">검색</button>
	</form>
	...
	<div class="pagination">
		...
		%>
		<a href="table2.jsp?curPage=<%=curPage%>">◀</a>
		<%
			for (int i = stratPage; i < stratPage + pageNum; i++) {
		%>
		<a href="table2.jsp?curPage=<%=i%>"><%=i%></a>
		<%
			if (i >= totalPage)
					break;
			}
...
<%=session.getAttribute("id") != null%>
			...
			location.href = '/Web_Project/include/iframe/write.jsp?category=2'
		} else {
			alert('로그인 후 이용가능합니다.');
		}
	}
